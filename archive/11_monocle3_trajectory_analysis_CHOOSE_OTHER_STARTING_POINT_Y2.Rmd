...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=18}
list_of_principal_points <- c("Y_2")
print(sprintf("List of principal points: %s", paste(list_of_principal_points, collapse = ", ")))
```
...
if (file.exists(file.path(path.to.11.output, "monocle.obj_graph_test_res.rds")) == FALSE){
  monocle.obj_graph_test_res <- graph_test(monocle.obj, neighbor_graph="principal_graph", cores=16)
  saveRDS(monocle.obj_graph_test_res, file.path(path.to.11.output, "monocle.obj_graph_test_res.rds"))
} else {
  monocle.obj_graph_test_res <- readRDS(file.path(path.to.11.output, "monocle.obj_graph_test_res.rds"))
}
...
#                               plot = TRUE,
#                               parallel = FALSE)
# saveRDS(eval_to_choose_K, file.path(path.to.11.output, "eval_to_choose_K.rds"))
```

```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14}
print("Start working on fitGAM...")
if (file.exists(file.path(path.to.11.output, "sce.rds")) == FALSE){
  ...
  saveRDS(sce, file.path(path.to.11.output, "sce.rds"))  
} else {
  sce <- readRDS(file.path(path.to.11.output, "sce.rds"))
}
```
...
##### Test within lineage
#####----------------------------------------------------------------------#####
if (file.exists(file.path(path.to.11.output, "tradeseq_test_res.rds")) == FALSE){
  ...
  saveRDS(tradeseq.testres, file.path(path.to.11.output, "tradeseq_test_res.rds"))
} else {
  tradeseq.testres <- readRDS(file.path(path.to.11.output, "tradeseq_test_res.rds"))
}
