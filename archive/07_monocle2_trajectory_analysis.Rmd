...
### State 1 vs State 2
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
pairwise.state.markers$`1_2` <- pairwise.state.markers$`1_2` %>%
...
pairwise.state.markers$`1_2` %>%create_dt()
```

### State 2 vs State 3
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
pairwise.state.markers$`2_3` <- pairwise.state.markers$`2_3` %>%
  rownames_to_column("Gene") %>%
  subset(p_val_adj <= 0.05) %>% 
  rowwise() %>%
  mutate(abs_log2FC = abs(avg_log2FC)) %>%
...
pairwise.state.markers$`2_3`%>% 
  create_dt()
```

### Genes that are differentially expressed between State 1 vs State 2 and State 2 vs State 3
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}

diff.genes.3.states <- intersect(pairwise.state.markers$`1_2`$Gene, pairwise.state.markers$`2_3`$Gene)

tmp1 <- subset(pairwise.state.markers$`1_2`, pairwise.state.markers$`1_2`$Gene %in% diff.genes.3.states) %>%
  subset(select = c(Gene, avg_log2FC, abs_log2FC))
colnames(tmp1) <- c("Gene", "avg_log2FC_1_vs_2", "abs_log2FC_1_vs_2")
tmp2 <- subset(pairwise.state.markers$`2_3`, pairwise.state.markers$`2_3`$Gene %in% diff.genes.3.states) %>%
  subset(select = c(Gene, avg_log2FC, abs_log2FC))
colnames(tmp2) <- c("Gene", "avg_log2FC_2_vs_3", "abs_log2FC_2_vs_3")


merge(tmp1, tmp2, by.x  = "Gene", by.y = "Gene") %>% create_dt()
```

## Differential gene expression test along trajectories (VGAM)
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
if (file.exists(file.path(path.to.07.output, "pseudotime.markers.rds")) == FALSE){
  pseudotime.markers <- differentialGeneTest(monocle.obj, fullModelFormulaStr = "~sm.ns(Pseudotime)", cores = 16, verbose = TRUE)
  saveRDS(pseudotime.markers, file.path(path.to.07.output, "pseudotime.markers.rds"))
} else {
  pseudotime.markers <- readRDS(file.path(path.to.07.output, "pseudotime.markers.rds"))
}
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
if (file.exists(file.path(path.to.07.output, "sce.rds")) == FALSE){
  ...
  sce <- readRDS(file.path(path.to.07.output, "sce.rds"))
}
```
...
##### Test within lineage
#####----------------------------------------------------------------------#####
if (file.exists(file.path(path.to.07.output, "tradeseq_test_res.rds")) == FALSE){
  ...
  saveRDS(tradeseq.testres, file.path(path.to.07.output, "tradeseq_test_res.rds"))
} else {
  tradeseq.testres <- readRDS(file.path(path.to.07.output, "tradeseq_test_res.rds"))
}
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
if (file.exists(file.path(path.to.07.output, "BEAM_res.rds")) == FALSE){
  BEAM_res <- BEAM(monocle.obj, branch_point = 1, cores = 10)
  BEAM_res <- BEAM_res[order(BEAM_res$qval),]
  BEAM_res <- BEAM_res[,c("gene_short_name", "pval", "qval")]
  saveRDS(BEAM_res, file.path(path.to.07.output, "BEAM_res.rds"))  
} else {
  BEAM_res <- readRDS(file.path(path.to.07.output, "BEAM_res.rds"))
}
...
p <- plot_genes_branched_heatmap(monocle.obj[selected.BEAM.genes$gene_short_name,],
                                 ...
                                 show_rownames = T, 
                                 return_heatmap = TRUE)
ggsave(plot = p$ph_res, filename = "BEAM_branch_DE_genes_top100.svg", device = "svg", path = file.path(path.to.07.output), dpi = 300, width = 20, height = 15)
```
