...
path.to.s.obj <- file.path(outdir, PROJECT, "data_analysis", "03_output", "20231018_SAlBounny.filter_contaminated_cells.clusterRes_0.5.subClusterFoxp3_cluster6.rds")

s.obj.name <- basename(path.to.s.obj)
...
plot_cells(monocle.obj, label_principal_points = TRUE, graph_label_size = 5, show_trajectory_graph = TRUE, cell_size = 1.5, color_cells_by = "seurat_clusters", 
...
list_of_principal_points <- c(get_earliest_principal_node(cds = monocle.obj, cluster.id = 7))
print(sprintf("List of principal points: %s", paste(list_of_principal_points, collapse = ", ")))



print("Manually select the prinipal points as Y_21")
list_of_principal_points <- c("Y_21")
...
if (file.exists(file.path(path.to.09.output, "monocle.obj_graph_test_res.rds")) == FALSE){
  monocle.obj_graph_test_res <- graph_test(monocle.obj, neighbor_graph="principal_graph", cores=16)
  saveRDS(monocle.obj_graph_test_res, file.path(path.to.09.output, "monocle.obj_graph_test_res.rds"))
} else {
  monocle.obj_graph_test_res <- readRDS(file.path(path.to.09.output, "monocle.obj_graph_test_res.rds"))
}
...
#                               plot = TRUE,
#                               parallel = FALSE)
# saveRDS(eval_to_choose_K, file.path(path.to.09.output, "eval_to_choose_K.rds"))


print("Start working on fitGAM...")
if (file.exists(file.path(path.to.09.output, "sce.rds")) == FALSE){
  ...
  saveRDS(sce, file.path(path.to.09.output, "sce.rds"))  
} else {
  sce <- readRDS(file.path(path.to.09.output, "sce.rds"))
}
