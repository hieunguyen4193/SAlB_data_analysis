...
coldf <- data.frame(celltype.cols) %>% t() %>% as.data.frame() %>% rownames_to_column("celltype")
colnames(coldf) <- c("celltype", "color")
write.csv(coldf, file.path(path.to.06.output, "color.csv"))
```

```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
DimPlot(object = s.obj, reduction = "INTE_UMAP", group.by = "plot.cluster", label = TRUE, label.box = TRUE, repel = TRUE, pt.size = 1, cols = unlist(celltype.cols))
```
