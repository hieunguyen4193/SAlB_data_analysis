...
# New UMAP plot with selected colors
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
meta.data <- s.obj@meta.data %>% rownames_to_column("barcode")
meta.data[[sprintf("cluster_%s", input.gene)]] <- factor(meta.data[[sprintf("cluster_%s", input.gene)]], levels = names(color.list))
meta.data <- meta.data %>% column_to_rownames("barcode")
meta.data <- meta.data[row.names(s.obj@meta.data),]
s.obj <- AddMetaData(object = s.obj, col.name = sprintf("cluster_%s.plot", input.gene), metadata = meta.data[[sprintf("cluster_%s", input.gene)]])
DimPlot(object = s.obj, 
        reduction = "INTE_UMAP", 
        label = TRUE, 
        label.box = TRUE, 
        repel = TRUE, 
        group.by = sprintf("cluster_%s", input.gene), 
        cols = unlist(color.list))
```
